<?php
include_once "php/includes.php";

$url = DOMAIN.'/system/call.php?comp=auth&subcomp=sesreq';
$usrid = request($url);

if ( $usrid && $usrid != 0 ){
  // bypass login screen if session started
  header( 'Location: /web/mainpage.php' );
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1">

  <title>CetinApp</title>

  <link href="/web/css/bootstrap.min.css" rel="stylesheet">
  <link href="/web/css/signin.css" rel="stylesheet">
  <script src="http://code.jquery.com/jquery-1.11.0.min.js"></script>
  <script src="http://code.jquery.com/jquery-migrate-1.2.1.min.js"></script>
  <script src="/web/js/bootstrap.min.js"></script>
  <script src="/web/js/script.js"></script>
</head>

<body>

  <div class="container">

    <form class="form-signin" role="form">
      <h3 class="form-signin-heading">Welcome to CetinApp</h3>
      <input type="email" class="form-control email" placeholder="Email address" required autofocus>
      <input type="password" class="form-control pass" placeholder="Password" required>
      <button class="btn btn-lg btn-primary btn-block submitbutton" type="button">Sign in</button>
    </form>

  </div> 

</body>
</html>
